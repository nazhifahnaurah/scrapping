...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.scrappings.update", [$scrapping->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <label for="kategori">Kategori*</label>
                <select class="form-control {{ $errors->has('kategori') ? 'is-invalid' : '' }}" name="kategori" id="kategori" required>
                    ...
                    @foreach(App\Result::KATEGORI_SELECT as $id => $kategori)
                        <option value="{{ $id }}" {{ ($id ? $scrapping->kategori : old('kategori')) == $id ? 'selected' : '' }}>{{ $kategori }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('ta') ? 'has-error' : '' }}">
                <label for="ta">ta*</label>
                    <select class="form-control {{ $errors->has('ta') ? 'is-invalid' : '' }}" name="ta" id="ta" required>
                        <option value disabled {{ old('ta', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                        @foreach(App\Result::TA_SELECT as $id => $ta)
                            <option value="{{ $id }}" {{ ($id ? $scrapping->ta : old('ta')) == $id ? 'selected' : '' }}>{{ $ta }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('ta'))
                        <p class="help-block">
                ...
                <div class="form-group {{ $errors->has('semester') ? 'has-error' : '' }}">
                    <label for="semester">semester*</label>
                        <select class="form-control {{ $errors->has('semester') ? 'is-invalid' : '' }}" name="semester" id="semester" required>
                            ...
                            @foreach(App\Result::SEMESTER_SELECT as $id => $semester)
                                <option value="{{ $id }}" {{ ($id ? $scrapping->semester : old('semester')) == $id ? 'selected' : '' }}>{{ $semester }}</option>
                            @endforeach
                        </select>
              ...
              <div class="form-group">
                    ...
                    <label class="custom-file-label" for="file">{{$scrapping->file}}</label>
                    </div>
                    @error('file')
